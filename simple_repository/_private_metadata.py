...
class PrivateMetadataMapping(JSONMapping):
    ...
    def _finalize_data(
        ...
        invalid_keys = ', '.join(sorted(repr(key) for key in data if not key.startswith('_')))
        if invalid_keys:
            raise ValueError(f"The following keys are invalid for private metadata: {invalid_keys}")
        return data
