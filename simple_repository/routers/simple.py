...
from ..simple.serializer import serialize


def build_router(repository: SimpleRepository) -> APIRouter:
    ...
    async def project_list(
        ...
        try:
            response_format = utils.select_response_format(
        ...
        serialized_project_list = serialize(
            page=project_list,
    ...
    @simple_router.get("/simple/{package_name}/")
    async def simple_project_page(
        ...
        if format:
            # Allow the consumer to request a format as a query string such as
            # {URL}?format=application/vnd.pypi.simple.v1+json
            # + in urls are interpreted as spaces by
        ...
        serialized_project_page = serialize(
    ...
    @simple_router.get("/resources/{package_name}/{resource_name}")
    async def resources(
        resource_name: str,
        package_name: str,
        request: fastapi.Request,
        ...
        if isinstance(resource, model.LocalResource):
            # Use the first 12 characters of the package digest as ETag
            file_hash = utils.sha256sum(file_path=resource.path)[:12]
            if file_hash == request.headers.get("if-none-match"):
                raise HTTPException(304)
            return FileResponse(
                path=resource.path,
